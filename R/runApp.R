...
#' Run BigcMetaExplorer web interface
#'
...
runApp <- function(cache_dir = "cache",
  ...
  appDir <- system.file("shinyapp", package = "BigcMetaExplorer")
  if (appDir == "") {
    stop("Could not find example directory. Try re-installing `BigcMetaExplorer`.", call. = FALSE)
  }
  ...
  shiny::shinyApp(BigcMetaExplorer::dashboardUI, BigcMetaExplorer::pavianServer, enableBookmarking=enableBookmarking, options = list(...))

  ## TODO: Restoring options like this does not work - we never get here after shiny::runApp as the server keeps running
  #options(old_options)
}

#' @describeIn runApp Create BigcMetaExplorer app (passes arguments to runApp)
#' @export
pavianApp <- function(...) {
    BigcMetaExplorer::runApp(...)
}
