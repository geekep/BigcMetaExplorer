...
serverDataPanel <- function(ns) {
  tabPanel(
    "Use data on server",
    id = "server_dir",
    " Select a directory in the file browser and press 'Read selected directories' to load it into BigcMetaExplorer.",
    ...
    shinyWidgets::searchInput(ns("search_data_dir"),
                              label = "Specify directory on machine running BigcMetaExplorer",
...
exampleDataPanel <- function(ns) {
  tabPanel(
    ...
    br(),
    br(),
    {if (!dir.exists(system.file("shinyapp", "example-data", package = "BigcMetaExplorer"))) {
...
#' UI part of BigcMetaExplorer data input module
#'
...
dataInputModuleUI <- function(id,
  ...
  shiny::tagList(box(
    ...
    HTML(
      "
      <p>
      BigcMetaExplorer is a tool for interactive analysis of metagenomics classification results. 
      It's built on <a href='https://www.r-project.org/' target='_blank'>R</a> and <a target='_blank' href='http://shiny.rstudio.com/'>Shiny</a>, and supports <a target='_blank' href='https://ccb.jhu.edu/software/kraken/'>Kraken</a>, <a target='_blank' href='https://github.com/infphilo/centrifuge'>Centrifuge</a> and <a target='_blank' href='https://bitbucket.org/biobakery/metaphlan2'>MetaPhlAn</a> report files.
      Please note that currently the default Centrifuge report format is not supported.
      To generate a compatible report, use the script centrifuge-kreport that is distributed with Centrifuge. 
      </p>
      <p>
      For help and more documenation please go to <a target='_blank' href='https://github.com/geekep/BigcMetaExplorer'>https://github.com/geekep/BigcMetaExplorer</a>.
      </p>"
    )
  ),
  {
...
#' Server part of BigcMetaExplorer data input module.
#'
...
dataInputModule <- function(input, output, session,
                            #server_dirs = c(pavian_lib_dir=system.file("shinyapp", "example-data", package = "BigcMetaExplorer"),
                            #                root = "/home/xuzhen"),
                            config_dir = NULL,
                            server_access = getOption("pavian.server_access", default = FALSE),
  ...
  observeEvent(input$btn_load_example_data, {
      read_server_directory(
        system.file("shinyapp", "example-data", package = "BigcMetaExplorer"),
  ...
  output$table <- renderRHandsontable({
    ...
    rh <-
      ...
      hot_col(
        ...
        #    cellProperties.comment = 'The file format does not validate. BigcMetaExplorer supports the outputs from kraken-report, centrifuge-kreport (but not the centrifuge --report-file!), and metaphlan2.py. You can create a valid centrifuge report with centrifuge-kreport -x IDX OUT_FILE.';
        #  }
        #  return td;
        #}"
      ) %>%
