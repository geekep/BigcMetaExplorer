...
pavianServer <- function(input, output, session) {
  ...
  output$bookmarkBtnUI <- renderUI ({
    ...
    shiny::tagList(
      bookmarkButton(id="bookmark_btn",label = "Bookmark state ...", title = "Bookmark this BigcMetaExplorer's state and get a URL for sharing."),
  ...
  generate_report_modal <- function() {
    ...
    if (rmarkdown::pandoc_available("1.12.3")) {
      modalDialog(
        ...
        textInput(ns("report_author"), "Author", sprintf("BigcMetaExplorer R package v%s", utils::packageVersion("BigcMetaExplorer")), width="100%"),
  ...
  output$dl_report <- downloadHandler(
    ...
    content = function(file) {
      req(input$sample_set_names)
      rmd_file <- system.file("pavian-report.Rmd", package="BigcMetaExplorer")
