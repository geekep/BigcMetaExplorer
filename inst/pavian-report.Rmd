...
params:
  ...
  doc_author: !r paste("generated with BigcMetaExplorer v",utils::packageVersion("BigcMetaExplorer"))
  doc_date: !r date()
...
if (isTRUE(params$all_data_loaded)) {
  ...
  sample_directory <- system.file("shinyapp","example-data","brain-biopsies",package="BigcMetaExplorer")
  sample_data <- BigcMetaExplorer::read_sample_data(sample_directory)
  reports <- BigcMetaExplorer::read_reports(sample_data$ReportFilePath, sample_data$Name)
}

samples_summary <- BigcMetaExplorer::summarize_reports(reports)
...
colnames(samples_summary) <- BigcMetaExplorer:::beautify_string(colnames(samples_summary))
...
tabsetPanel(
    tabPanel("Classification summary",
           DT::datatable(samples_summary_percent,   
              ...
              class = "stripe hover row-border compact",
              extensions = 'Buttons', 
              options = list( dom = 'Bfrtip', buttons = BigcMetaExplorer:::common_buttons(set_name))) %>% BigcMetaExplorer:::formatSummaryDT(display_percent = TRUE)
  ), 
  tabPanel("Raw read numbers",
    DT::datatable(samples_summary,  
                ...
                class = "stripe hover row-border compact",
                extensions = 'Buttons', 
                options = list( dom = 'Bfrtip', buttons = BigcMetaExplorer:::common_buttons(set_name))) %>% BigcMetaExplorer:::formatSummaryDT(display_percent = FALSE)
  ),
  tabPanel("Sample information",
           DT::datatable(sample_data,   
              class = "stripe hover row-border compact nowrap",
              extensions = 'Buttons', 
              options = list( dom = 'Bfrtip', buttons = BigcMetaExplorer:::common_buttons(set_name))))
)
```
...
merged_reports <- BigcMetaExplorer::merge_reports2(reports, col_names = sample_data$Name)
...
my_df <- data.frame(Name=tax_data$name,Max=apply(cladeReads,1,max,na.rm=TRUE),cladeReads,Lineage=BigcMetaExplorer:::beautify_taxLineage(tax_data$taxLineage), stringsAsFactors = FALSE)
my_dt <- function(sel) {
  selw <- which(sel)
  ...
  shiny::tagList(
    ...
    DT::datatable(my_df[selw,,drop=F],height = "600px", rownames = FALSE,
              class = "stripe hover row-border compact nowrap", extensions = 'Buttons',  options = list( dom = 'Bfrtip', buttons = BigcMetaExplorer:::common_buttons(set_name), scrollX = TRUE))
  )
}
tabsetPanel(
...
#             class = "stripe hover row-border compact nowrap",
#              extensions = 'Buttons', 
#              options = list( dom = 'Bfrtip', buttons = BigcMetaExplorer:::common_buttons(set_name), scrollX = TRUE)))
#)
```
...
#  
#   ac <- ac %>% bs_append(title_side=n,content_side="",content_main=BigcMetaExplorer:::build_sankey_network(my_report2, nodePadding=13, xScalingFactor=.9, nodeStrokeWidth=0, zoom = FALSE, colourScale = colourScale) )
# }
# 
...
#  n="PT1"
  ...
  #    tabPanel("Sankey visualization",
        ...
        BigcMetaExplorer:::build_sankey_network(my_report, nodePadding=13, xScalingFactor=.9, nodeStrokeWidth=0, zoom = FALSE, colourScale = "sankey_colorscale", LinkGroup = "source_name")
...
About
=====
This file was generated with the BigcMetaExplorer R package version `r utils::packageVersion('BigcMetaExplorer')` on `r date()`. Please cite BigcMetaExplorer if you use it in your research.
